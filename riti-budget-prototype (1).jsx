...
/* =========================================================================
   GEMINI INTEGRATION
   SECURITY NOTE: this key is hardcoded here for local/private prototyping
   only. Because this is client-side code, anyone who opens this app in a
   browser can view the key in the page source or network tab. Do not
   deploy this file to a public URL as-is. For anything beyond your own
   local testing, move this call behind a small backend that holds the
   key server-side instead.

   Design choice: Gemini is used only to phrase the natural-language
   explanation of an answer. The actual figures (costs, projections,
   utilization) are always computed locally by the tested engine below
   and passed to Gemini as fixed facts, so the model explains numbers,
   it never invents them.
   ========================================================================= */

async function askGemini(question, dataSummary, computedAnswer) {
  const prompt = `You are a budget assistant inside an Indian state government planning tool. Answer the user's question in plain, natural language (no markdown, no headers), in under 55 words.

Rules:
- Use ONLY the figures given below. Never invent, estimate, or adjust any number yourself.
- The "computed result" section already has the correct answer worked out. Your job is only to phrase it clearly and naturally, referencing the actual numbers given.
- Do not add caveats about being an AI or mention data sources.
...
User's question: ${question}`;
  ...
  const res = await fetch("/api/ask", {
    ...
    body: JSON.stringify({ question, dataSummary, computedAnswer }),
  });
  if (!res.ok) {
  ...
  return { text: text.trim(), sources: data.sources || [] };
}
...
function buildDataSummary(ds, scopeDeptId) {
  ...
  depts.forEach((d) => {
    ...
    (ds.schemesByDept[d.id] || []).forEach((s) => {
      const hist = ds.sanctioned.filter((r) => r.scheme === s.id).map((r) => `${r.year}: Rs ${r.sanctioned} Cr`).join(", ");
      lines.push(`  - ${s.name} (${s.category}): ${hist || "no data"}`);
    });
  });
  return lines.join("\n");
}
...
/* =========================================================================
   PART 1: DEMO DATASET (used until the user uploads their own)
   Same synthetic Rajasthan-style data as before. Structured exactly like
   what an uploaded file would produce after mapping, so the exact same
   engine in Part 3 runs on both.
   ========================================================================= */
...
/* =========================================================================
   PART 2: DATA SOURCE ABSTRACTION
   Everything downstream (charts, Q&A, projections, builder) reads through
   this shape, regardless of whether it came from the demo dataset or an
   uploaded file. This is what makes "upload your data and get results"
   real rather than cosmetic: the exact same functions in Part 3 run on
   whichever warehouse is active.

   Shape:
   {
     departments: [{ id, name, sector, color }],
     schemesByDept: { deptId: [{ id, name, category, unitLabel?, base, unitCost? }] },
     years: [...],
     yearsActual: [...],
     sanctioned: [{ dept, scheme, year, sanctioned, category, deptName, schemeName }],
     actual: [{ ...sanctioned row, actual, utilization }],
     unitCosts: { schemeId: { material, labor, contingencyPct, unitLabel } },
     isUploaded: boolean,
     districts: [...]
   }
   ========================================================================= */
...
/* ---- Column detection heuristics for uploaded files ---- */
...
function parseUploadedRows(rows, mapping) {
  // rows: array of objects from Papa/XLSX with original headers
  ...
  rows.forEach((row) => {
    ...
    const schemeId = `${deptId}__${schemeName.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "")}` || `${deptId}__scheme`;
    ...
    if (!isNaN(sanctionedVal)) {
      sanctioned.push({ dept: deptId, deptName, scheme: schemeId, schemeName, category, year, sanctioned: sanctionedVal, unitCount: isNaN(unitCount) ? null : unitCount });
    }
    if (!isNaN(actualVal)) {
  ...
  // merge actual into sanctioned rows where year+scheme match, and compute utilization
  ...
  // derive a rough base (average sanctioned) per scheme for trend math when history is thin
...
/* =========================================================================
   PART 3: THE ENGINE
   Every function here takes a `ds` (data source) as its first argument,
   so it works identically whether ds is the demo warehouse or a freshly
   uploaded and mapped file.
   ========================================================================= */
...
function answerQuestion(ds, query, scopedDeptId) {
  ...
  if (/\d+/.test(t) && findSchemeByFuzzyMatch(ds, t)) {
    ...
    if (!est.error) {
      return {
        kind: "estimate",
        text: `${est.count} ${est.cost.unitLabel}${est.count > 1 ? "s" : ""} of ${est.scheme.name.toLowerCase()} would cost approximately ${CR(est.cost.totalCr)} at current rates. Accounting for typical spending patterns, a realistic budget to set aside is ${CR(est.aspiration.realistic)}.`,
...
/* =========================================================================
   DATA SOURCE CONTEXT (simple prop-drilling root, no external state lib needed)
   ========================================================================= */
  ...
  if (ext === "csv") {
    ...
    reader.onload = (e) => {
      try {
        ...
        onDone(json, headers);
      } catch (err) { onError("Could not read this file. Check that it's a valid Excel file."); }
    };
...
function UploadView({ onDataReady }) {
  const [stage, setStage] = useState("idle"); // idle | mapping | error
    ...
    <Card style={{ maxWidth: 620, margin: "0 auto", textAlign: "center" }}>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: MUTE, marginBottom: 18, maxWidth: 380, marginInline: "auto", lineHeight: 1.6 }}>
          A CSV or Excel file with columns like department, scheme, year, and sanctioned amount. We'll ask you to confirm what each column means, then the same analysis engine runs on your figures.
        </div>
...
function AskWidget({ scopeLabel, scopeDeptId, suggestions, accentColor = "#2E5FB5" }) {
  ...
  async function ask(q) {
    ...
    try {
      ...
      const computed = summarizeComputedAnswer(localAnswer);
      const phrased = await askGemini(text, summary, computed);
      finalAnswer = { ...localAnswer, text: phrased.text, sources: phrased.sources };
    } catch (e) {
      // Gemini call failed (network, quota, bad key, etc). Fall back to the
      // locally computed explanation so the widget never breaks.
      finalAnswer = localAnswer;
    }
    setThread((t) => [...t, { q: text, a: finalAnswer }]);
    setThinking(false);
  }
  ...
  return (
    <Card padding={0} style={{ overflow: "hidden" }}>
      ...
      <div style={{ padding: 18 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: thread.length || thinking ? 14 : 0 }}>
          {thread.map((item, i) => (
            <div key={i}>
...
/* =========================================================================
   MICRO CALCULATOR (construction / unit-cost quick tool)
   ========================================================================= */
...
function MicroCalculator() {
  ...
  const scheme = unitCostSchemes.find((s) => s.id === schemeId);
...
function SectorView() {
  ...
  const projection = useMemo(() => (sector ? projectDepartmentBudget(ds, sectorId) : null), [ds, sectorId]);
...
function BudgetBuilder() {
  ...
  const dept = ds.departments.find((d) => d.id === deptId);
  const projection = useMemo(() => (dept ? projectDepartmentBudget(ds, deptId) : null), [ds, deptId]);
